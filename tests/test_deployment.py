...
def assert_json_response(
    response: Any, expected_status: int, expected_data: Optional[Dict] = None
) -> None:
...
def make_invoke_request(
    client: TestClient, input_text: str, stream: bool = False
) -> Any:
...
class BaseStreamResult:
    ...
    def create_event(delta: str) -> MagicMock:
        return MagicMock(
            type="raw_response_event", data=MagicMock(delta=f"data: {delta}\n\n")
        )
...
def test_agent():
    ...
    return Agent(name=TEST_AGENT_NAME, instructions=TEST_AGENT_INSTRUCTIONS)
...
class TestBasicEndpoints:
    ...
    def test_health_check(self, test_client):
        ...
        expected_data = {"status": "healthy", "agent": TEST_AGENT_NAME}
    ...
    def test_agent_info(self, test_client):
        ...
        expected_data = {"name": TEST_AGENT_NAME, "type": "Agent"}
        assert_json_response(response, 200, expected_data)
...
class TestSyncInvoke:
    ...
    def test_invoke_with_various_inputs(
        self, test_client, mock_run, test_agent, input_text, description
    ):
